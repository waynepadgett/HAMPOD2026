 ...
 * The pipeline is started once at init and text is written to it for immediate
 * playback.
 */
...
int hal_tts_init(void) {
  ...
  snprintf(command, sizeof(command),
           "piper --model %s --length_scale %s --output_raw 2>/dev/null | "
           "aplay -D %s -r 16000 -f S16_LE -t raw - 2>/dev/null",
...
int hal_tts_speak(const char *text, const char *output_file) {
  ...
  /* Auto-reinitialize if we were interrupted or not initialized */
...
void hal_tts_interrupt(void) {
  if (persistent_pipe) {
    ...
    initialized = 0;
    printf("HAL TTS: Interrupted (closed pipeline)\n");
  }
}
...
const char *hal_tts_get_impl_name(void) { return "Piper (Persistent)"; }
