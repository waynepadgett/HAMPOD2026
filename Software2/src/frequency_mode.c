...
bool frequency_mode_handle_key(char key, bool is_hold) {
  ...
  case FREQ_MODE_SELECT_VFO:
    ...
    if (key == '*') {
      // TODO: Instead of cancelling, treat first * as a leading
      // zero followed by a decimal point (i.e. "0.") and transition
      // to FREQ_MODE_ENTERING. This matches how users would enter
      // sub-1 MHz frequencies like 0.472 MHz.
      // Cancel
      clear_freq_buffer();
      g_state = FREQ_MODE_IDLE;
...
bool frequency_mode_is_active(void) { return g_state != FREQ_MODE_IDLE; }

FreqModeState frequency_mode_get_state(void) { return g_state; }
...
void frequency_mode_on_radio_change(double new_freq) {
  ...
  if (g_state == FREQ_MODE_IDLE) {
    if (g_suppress_next_poll) {
      DEBUG_PRINT(
          "frequency_mode_on_radio_change: Suppressed (we just set it)\n");
    ...
    if (!normal_mode_get_verbosity()) {
      DEBUG_PRINT(
          "frequency_mode_on_radio_change: Suppressed (verbosity off)\n");
      return;
    }
